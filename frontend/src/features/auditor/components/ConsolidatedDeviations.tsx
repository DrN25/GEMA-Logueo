    ...
    <div className="rounded-xl border border-cyan-500/10 bg-[#090f1d]/50 p-6 space-y-6 shadow-xl relative overflow-hidden">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <button
          ...
          className="flex items-center gap-1.5 bg-cyan-500 hover:bg-cyan-600 border border-cyan-400/30 text-slate-950 px-4 py-2 rounded-lg text-xs font-black transition-all shadow-[0_0_12px_rgba(6,182,212,0.15)] active:scale-95"
        >
          {isConsolidatedExpanded ? (
            <>
              <EyeOff size={14} />
            ...
            <>
              <Eye size={14} />
              <span>Mostrar Panel de Consolidado</span>
            </>
          )}
        </button>
      </div>
          ...
          {!selectedObservation ? (
            <div className="space-y-4">
              <p className="text-xs text-slate-400 font-semibold">
                Haz clic sobre una tipología para desplegar tendencias e identificar taladros anómalos.
              </p>
