...
def save_taladro(taladro: TaladroSchema, db: Session = Depends(get_db)):
    ...
    for c in taladro.corridas:
        ...
        param = models.ParametrosTaladroLG(
            ...
            abertura_mm=None if c.abertura == -1 else c.abertura,
            rugosidad_isrm=None if c.rugosidad == -1 else c.rugosidad,
            sum_frac_nat=c.frac_nat,
            jrc_10=None if c.jrc10 == -1 else c.jrc10,
            tipo_de_relleno_1_code=c.relleno1 if c.relleno1 != "-1" else None,
            espesor_relleno_mm=None if c.espesor == -1 else c.espesor,
    ...
    for d in taladro.discontinuidades:
        ...
        if reg_id:
            ...
            struct_log = models.DatosLogueoEstructural(
                ...
                alpha=float(d.alfa) if (d.alfa is not None and d.alfa != -1.0) else None,
                beta=float(d.beta) if (d.beta is not None and d.beta != -1.0) else None,
                dip=None,
                azimuth=None,
                rugosidad_codigo=None if d.forma == -1 else d.forma,
                jrc=None if d.jrc10 == -1 else d.jrc10,
                abertura_mm=None if d.abertura == -1 else d.abertura,
                espesor_relleno_mm=None if d.espesor == -1 else d.espesor,
